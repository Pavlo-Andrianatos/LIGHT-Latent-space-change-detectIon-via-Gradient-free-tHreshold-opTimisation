...
def displayGenerated(model_output, i, k, loaded_model_name, load_models_file_path, dataset_used, semantic=False,
    ...
    if not semantic:
        if not test_change:
            path = "{}/{}/result/train/threshold/ga/argmax/argmax_{}_{}.png".format(
                load_models_file_path, loaded_model_name, i, k)
        else:
            path = "{}/{}/result/test/threshold/ga/argmax/argmax_{}_{}.png".format(
                load_models_file_path, loaded_model_name, i, k)
        ...
        if not test_change:
            path = "{}/{}/result/train/threshold/ga/predicted_change/predicted_change_{}_{}.png".format(
                load_models_file_path, loaded_model_name, i, k)
        else:
            path = "{}/{}/result/test/threshold/ga/predicted_change/predicted_change_{}_{}.png".format(
                load_models_file_path, loaded_model_name, i, k)
        ...
        if not test_change:
            path = "{}/{}/result/train/threshold/ga/argmax/argmax_semantic_{}_{}_{}.png".format(
                load_models_file_path, loaded_model_name, semantic_number, i, k)
        else:
            path = "{}/{}/result/test/threshold/ga/argmax/argmax_semantic_{}_{}_{}.png".format(
                load_models_file_path, loaded_model_name, semantic_number, i, k)
...
def displayTruth(y_truth, i, k, loaded_model_name, load_models_file_path, dataset_used, test_change=False):
    ...
    if not test_change:
        imageio.imwrite("{}/{}/result/train/threshold/ga/gtruth/gtruth_change_{}_{}.png".format(
            load_models_file_path, loaded_model_name, i, k), dis)
    else:
        imageio.imwrite("{}/{}/result/test/threshold/ga/gtruth/gtruth_change_{}_{}.png".format(
            load_models_file_path, loaded_model_name, i, k), dis)
...
def train_Thresholds_Function(generator, did_load_model, parametersObject, param=None):
    ...
    output_every_x_images = parametersObject["output_every_x_images"] # Output every x image
    ...
    print("GA Training is commencing...")
    ...
    def calculateScore(model_output, change_image_ga, dataset_used_score, all_metrics=False):
        change_image_ga = change_image_ga.cpu().detach().numpy()

        change_image_ga = change_image_ga.squeeze(0)

        change_image_ga = change_image_ga.astype(np.uint8)
        ...
        if dataset_used_score == "vaihingen":
            ...
            y_argmax_val[change_image_ga == 0] = 0
            y_argmax_val[y_argmax_val == 2] = 1

        change_image_ga = change_image_ga.transpose(1, 2, 0)

        change_image_ga = change_image_ga.squeeze(2)

        true_positives = np.sum(np.logical_and(y_argmax_val == 1, change_image_ga == 1), dtype='int64')
        true_negatives = np.sum(np.logical_and(y_argmax_val == 0, change_image_ga == 0), dtype='int64')
        false_positives = np.sum(np.logical_and(y_argmax_val == 1, change_image_ga == 0), dtype='int64')
        false_negatives = np.sum(np.logical_and(y_argmax_val == 0, change_image_ga == 1), dtype='int64')
    ...
    def fitness_func(solution, solution_idx):
        model.train(False)
        total_difference_score_ga = 0
        total_number_tested = 0
        for l_index_1_ga, (image_ga, second_image_ga, change_image_ga) in enumerate(train_data_loader):
            image_ga = image_ga.to(device, non_blocking=True)
            second_image_ga = second_image_ga.to(device, non_blocking=True)

            with torch.no_grad():
                if not is_running_baseline:
                    y_ga = model.forward(inputImage=image_ga, secondImage=second_image_ga,
                                         threshold_down1=float(solution[0]),
                                         threshold_down2=float(solution[1]),
                                         threshold_down3=float(solution[2]),
                                         threshold_down4=float(solution[3]),
                                         threshold_bridge=float(solution[4]))
            ...
            score_ga = calculateScore(copy.deepcopy(y_ga), copy.deepcopy(change_image_ga),
                                      dataset_used)

            total_number_tested += 1

            total_difference_score_ga = total_difference_score_ga + score_ga

        difference_value_train = (total_difference_score_ga / total_number_tested)
        ...
        for l_index_1_val, (image_val, second_image_val, change_image_val) in enumerate(validation_data_loader):
            ...
            with torch.no_grad():
                if not is_running_baseline:
                    y_ga_val = model.forward(inputImage=image_val, secondImage=second_image_val,
                                             threshold_down1=float(solution[0]),
                                             threshold_down2=float(solution[1]),
                                             threshold_down3=float(solution[2]),
                                             threshold_down4=float(solution[3]),
                ...
                score_ga_val = calculateScore(copy.deepcopy(y_ga_val), copy.deepcopy(change_image_val),
                                              dataset_used, True)

            if output_images == "True":
                if number_of_classes == 3:
                    v_utils.save_image(copy.deepcopy(y_ga_val),
                                       "{}/{}/result/train/threshold/ga/argmax/gen_image_{}_{}.png".format(
                                           path_to_models,
                                           loaded_model_name,
                                           Utils.global_ga_iteration_number, l_index_1_val))

                displayGenerated(copy.deepcopy(y_ga_val), Utils.global_ga_iteration_number, l_index_1_val,
                                 loaded_model_name,
                                 path_to_models, dataset_used, change_image_pso=copy.deepcopy(change_image_val))

                displayGenerated(y_before, Utils.global_ga_iteration_number, l_index_1_val, loaded_model_name,
                                 path_to_models, dataset_used, semantic=True, semantic_number=0)
                displayGenerated(y_after, Utils.global_ga_iteration_number, l_index_1_val, loaded_model_name,
                                 path_to_models, dataset_used, semantic=True, semantic_number=1)

                displayTruth(copy.deepcopy(change_image_val), Utils.global_ga_iteration_number, l_index_1_val,
                             loaded_model_name,
                             path_to_models, dataset_used)
                v_utils.save_image(image_val.cpu().data,
                                   "{}/{}/result/train/threshold/ga/before/before_image_{}_{}.png".format(
                                       path_to_models,
                                       loaded_model_name,
                                       Utils.global_ga_iteration_number, l_index_1_val))
                v_utils.save_image(second_image_val.cpu().data,
                                   "{}/{}/result/train/threshold/ga/after/after_image_{}_{}.png".format(
                                       path_to_models,
                                       loaded_model_name,
                                       Utils.global_ga_iteration_number, l_index_1_val))
        ...
        print("Validation Score: " + str(difference_value_val) + ", for Iteration: " +
              str(Utils.global_ga_iteration_number) + ", higher is better.")
            ...
            if output_images == "True":
                if number_of_classes == 3:
                    v_utils.save_image(copy.deepcopy(y_test),
                                       "{}/{}/result/test/threshold/ga/argmax/gen_image_{}_{}.png".format(
                                           path_to_models,
                                           loaded_model_name,
                                           Utils.global_ga_iteration_number, l_index_1))

                displayGenerated(copy.deepcopy(y_test), Utils.global_ga_iteration_number, l_index_1,
                                 loaded_model_name,
                                 path_to_models, dataset_used, change_image_pso=copy.deepcopy(change_image_test),
                                 test_change=True)

                displayGenerated(y_before_test, Utils.global_ga_iteration_number, l_index_1, loaded_model_name,
                                 path_to_models, dataset_used, semantic=True, semantic_number=0, test_change=True)
                displayGenerated(y_after_test, Utils.global_ga_iteration_number, l_index_1, loaded_model_name,
                ...
                v_utils.save_image(image_test.cpu().data,
                                   "{}/{}/result/test/threshold/ga/before/before_image_{}_{}.png".format(
                                       path_to_models,
                                       loaded_model_name,
                                       Utils.global_ga_iteration_number, l_index_1))
                v_utils.save_image(second_image_test.cpu().data,
                                   "{}/{}/result/test/threshold/ga/after/after_image_{}_{}.png".format(
                                       path_to_models,
                                       loaded_model_name,
                                       Utils.global_ga_iteration_number, l_index_1))
        ...
        threshold_values_to_write = "Iteration {}\nthreshold_value_1 = {}\nthreshold_value_2 = {}\nthreshold_value_3 = {}\nthreshold_value_4 = {}\nthreshold_value_5 = {}\n\n".format(
            Utils.global_ga_iteration_number, solution[0], solution[1], solution[2],
            solution[3],
            solution[4])
        ...
        if param is not None:
            with open('{}/{}/GA_pyHopper_parameters.txt'.format(path_to_models, loaded_model_name),
                      "a") as pyHopper_parameters_val:
                pyHopper_parameters_val.write(str(param))
                pyHopper_parameters_val.write("\n")

        with open('{}/{}/GA Train Info about Models.txt'.format(path_to_models, loaded_model_name),
                  'a') as f_ga_info:
            f_ga_info.write("Iteration {} Training Metrics: "
                            "\n\tSeed: {}"
                            "\n\tTraining Score: {}"
                            "\n\tValidation Score: {}"
                            "\n\tTest Score: {}"
                            ...
                            "\n\tSeK: {}"
                            "\n\tscore: {}"
                            "\n\n".format(Utils.global_ga_iteration_number, seed,
                                          difference_value_train, difference_value_val, difference_value_test,
            ...
            f_ga_info.write("Iteration {} Test Metrics: "
                            "\n\tSeed: {}"
                            "\n\tTraining Score: {}"
                            "\n\tValidation Score: {}"
                            "\n\tTest Score: {}"
        ...
        Utils.global_ga_iteration_number = Utils.global_ga_iteration_number + 1

        return difference_value_train

    if param is not None:
        parent_selection_type = param["parent_selection_type"]

        crossover_type = param["crossover_type"]

        mutation_type = param["mutation_type"]
    else:
    ...
    def on_generation(ga_instance_var):
        print("Generation = {generation}\n".format(generation=ga_instance_var.generations_completed))
        print("Fitness    = {fitness}\n".format(
            fitness=ga_instance_var.best_solution(pop_fitness=ga_instance_var.last_generation_fitness)[1]))

    fitness_function = fitness_func

    initial_population = np.random.random_sample((4, 5))

    ga_instance = pygad.GA(num_generations=number_of_epochs,
    ...
    solution_best, solution_fitness_best, solution_idx_best = ga_instance.best_solution()

    print("Parameters of the best solution : {}".format(solution_best))
    print("Fitness value of the best solution = {}".format(solution_fitness_best))

    return solution_fitness_best
