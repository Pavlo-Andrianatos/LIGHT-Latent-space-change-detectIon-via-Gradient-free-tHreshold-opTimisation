...
def displayGenerated(model_output, i, k, loaded_model_name, load_models_file_path, dataset_used, semantic=False,
    ...
    if not semantic:
        if not test_change:
            path = "{}/{}/result/train/threshold/pso/argmax/argmax_{}_{}.png".format(
                load_models_file_path, loaded_model_name, i, k)
        else:
            path = "{}/{}/result/test/threshold/pso/argmax/argmax_{}_{}.png".format(
                load_models_file_path, loaded_model_name, i, k)
        ...
        if not test_change:
            path = "{}/{}/result/train/threshold/pso/predicted_change/predicted_change_{}_{}.png".format(
                load_models_file_path, loaded_model_name, i, k)
        else:
            path = "{}/{}/result/test/threshold/pso/predicted_change/predicted_change_{}_{}.png".format(
                load_models_file_path, loaded_model_name, i, k)

        image_to_save_classify = image_to_save_classify.astype(np.uint8)
        ...
        if not test_change:
            path = "{}/{}/result/train/threshold/pso/argmax/argmax_semantic_{}_{}_{}.png".format(
                load_models_file_path, loaded_model_name, semantic_number, i, k)
        else:
            path = "{}/{}/result/test/threshold/pso/argmax/argmax_semantic_{}_{}_{}.png".format(
                load_models_file_path, loaded_model_name, semantic_number, i, k)

        imageio.imwrite(path, image_to_save_classify)
...
def displayTruth(y_truth, i, k, loaded_model_name, load_models_file_path, dataset_used, test_change=False):
    ...
    if not test_change:
        imageio.imwrite("{}/{}/result/train/threshold/pso/gtruth/gtruth_change_{}_{}.png".format(
            load_models_file_path, loaded_model_name, i, k), dis)
    else:
        imageio.imwrite("{}/{}/result/test/threshold/pso/gtruth/gtruth_change_{}_{}.png".format(
            load_models_file_path, loaded_model_name, i, k), dis)
...
def train_Thresholds_Function(generator, did_load_model, parametersObject, param=None):
    ...
    number_of_change_images = parametersObject["number_of_change_images"]  # 20ish
    fitness_function = parametersObject["fitness_function"]

    print("PSO Training is commencing...")
    ...
    def custom_pso_cost_function(x):
        model.train(False)
        ...
        for l_index_1_pso, (image_pso, second_image_pso, change_image_pso) in enumerate(train_data_loader):
            ...
            total_difference_score_pso = total_difference_score_pso + score_pso

        difference_value = (total_difference_score_pso / total_number_tested)

        difference_value = (1 - difference_value)
        return np.array(
            [difference_value, difference_value, difference_value, difference_value, difference_value])

    def calculateScore(model_output, change_image, dataset_used_score, all_metrics=False):
        change_image_pso = change_image.cpu().detach().numpy()

        change_image_pso = change_image_pso.squeeze(0)

        if dataset_used_score == "second":
            change_image_pso[change_image_pso == 1] = 0
            change_image_pso[change_image_pso > 0] = 1
    ...
    if param is not None:
        my_options = {'c1': param["c1"], 'c2': param["c2"], 'w': param["w"]}
    else:
        my_options = {'c1': 0.5, 'c2': 0.5, 'w': 0.5}

    iteration_number = 0

    my_topology = Star()

    my_swarm = pyswarms.backend.create_swarm(n_particles=5, dimensions=1, options=my_options)

    pso_iterations = number_of_epochs
    for h in range(pso_iterations):
        my_swarm.current_cost = custom_pso_cost_function(my_swarm.position)
        my_swarm.pbest_cost = custom_pso_cost_function(my_swarm.pbest_pos)

        my_swarm.pbest_pos, my_swarm.pbest_cost = pyswarms.backend.compute_pbest(my_swarm)
        ...
        for l_index_1, (image_val, second_image_val, change_image_val) in enumerate(validation_data_loader):
            ...
            with torch.no_grad():
                if not is_running_baseline:
                    y = model.forward(inputImage=image_val, secondImage=second_image_val,
                                      threshold_down1=float(my_swarm.pbest_pos[0]),
                                      threshold_down2=float(my_swarm.pbest_pos[1]),
                                      threshold_down3=float(my_swarm.pbest_pos[2]),
                                      threshold_down4=float(my_swarm.pbest_pos[3]),
                                      threshold_bridge=float(my_swarm.pbest_pos[4]))
            ...
            if output_images == "True":
                rgb_tensor = y[:, 0:3, :, :]  # Shape: (1, 1, 320, 320)
                v_utils.save_image(rgb_tensor, "{}/{}/result/train/threshold/pso/argmax/gen_image_{}_{}.png".format(
                    path_to_models,
                    loaded_model_name,
                    iteration_number, l_index_1))

                displayGenerated(copy.deepcopy(y), iteration_number, l_index_1, loaded_model_name,
                                 path_to_models, dataset_used, change_image_pso=copy.deepcopy(change_image_val))

                displayGenerated(y_before, iteration_number, l_index_1, loaded_model_name,
                                 path_to_models, dataset_used, semantic=True, semantic_number=0)
                displayGenerated(y_after, iteration_number, l_index_1, loaded_model_name,
                                 path_to_models, dataset_used, semantic=True, semantic_number=1)

                displayTruth(copy.deepcopy(change_image_val), iteration_number, l_index_1, loaded_model_name,
                             path_to_models, dataset_used)
                v_utils.save_image(image_val.cpu().data,
                                   "{}/{}/result/train/threshold/pso/before/before_image_{}_{}.png".format(
                                       path_to_models,
                                       loaded_model_name,
                                       iteration_number, l_index_1))
                v_utils.save_image(second_image_val.cpu().data,
                                   "{}/{}/result/train/threshold/pso/after/after_image_{}_{}.png".format(
                                       path_to_models,
                                       loaded_model_name,
                                       iteration_number, l_index_1))
            ...
            if not total_difference_metrics:
                total_difference_metrics = score_val
            else:
                total_difference_metrics = [sum(x) for x in zip(total_difference_metrics, score_val)]
        ...
        print("Validation Score: " + str(difference_value_val) + ", for Iteration: " +
              str(iteration_number) + ", higher is better.")
        ...
        for l_index_1, (image_test, second_image_test, change_image_test) in enumerate(test_data_loader):
            ...
            if output_images == "True":
                rgb_tensor = y_test[:, 0:3, :, :]
                v_utils.save_image(rgb_tensor, "{}/{}/result/test/threshold/pso/argmax/gen_image_{}_{}.png".format(
                    path_to_models,
                    loaded_model_name,
                    iteration_number, l_index_1))

                displayGenerated(copy.deepcopy(y_test), iteration_number, l_index_1,
                                 loaded_model_name,
                                 path_to_models, dataset_used, change_image_pso=copy.deepcopy(change_image_test),
                                 test_change=True)

                displayGenerated(y_before_test, iteration_number, l_index_1, loaded_model_name,
                                 path_to_models, dataset_used, semantic=True, semantic_number=0, test_change=True)
                displayGenerated(y_after_test, iteration_number, l_index_1, loaded_model_name,
                                 path_to_models, dataset_used, semantic=True, semantic_number=1, test_change=True)

                displayTruth(copy.deepcopy(change_image_test), iteration_number, l_index_1,
                             loaded_model_name,
                             path_to_models, dataset_used, test_change=True)
                v_utils.save_image(image_test.cpu().data,
                                   "{}/{}/result/test/threshold/pso/before/before_image_{}_{}.png".format(
                                       path_to_models,
                                       loaded_model_name,
                                       iteration_number, l_index_1))
                v_utils.save_image(second_image_test.cpu().data,
                                   "{}/{}/result/test/threshold/pso/after/after_image_{}_{}.png".format(
                                       path_to_models,
                                       loaded_model_name,
                                       iteration_number, l_index_1))
        ...
        threshold_values_to_write = "Iteration {}\nthreshold_value_1 = {}\nthreshold_value_2 = {}\nthreshold_value_3 = {}\nthreshold_value_4 = {}\nthreshold_value_5 = {}\n\n".format(
            iteration_number, my_swarm.pbest_pos[0], my_swarm.pbest_pos[1], my_swarm.pbest_pos[2],
            my_swarm.pbest_pos[3],
            my_swarm.pbest_pos[4])
        ...
        if param is not None:
            with open('{}/{}/PSO_pyHopper_parameters.txt'.format(path_to_models, loaded_model_name),
                      "a") as pyHopper_parameters:
                pyHopper_parameters.write(str(param))
                pyHopper_parameters.write("\n")

        with open('{}/{}/PSO Train Info about Models.txt'.format(path_to_models, loaded_model_name),
                  'a') as f:
            f.write("Iteration {} Training Metrics: "
                    "\n\tSeed: {}"
                    "\n\tValidation Score: {}"
                    "\n\tTest Score: {}"
                    "\n\tPrecision: {}"
    ...
    print('The best cost found by our swarm is: {}, lower is better.'.format(my_swarm.best_cost))
    print('The best position found by our swarm is: {}'.format(my_swarm.pbest_pos))
