...
def train_method_pyhopper(param):
    ...
    Utils.global_pyhopper_iteration_number += 1
...
def displayGenerated(model_output, i, k, loaded_model_name, load_models_file_path, dataset_used, semantic=False,
    ...
    if dataset_used == "syn":
        image_to_save_classify[np.where((y_argmax_val == 0).all(axis=0))] = [225, 225, 85]
        image_to_save_classify[np.where((y_argmax_val == 1).all(axis=0))] = [65, 200, 75]
        image_to_save_classify[np.where((y_argmax_val == 2).all(axis=0))] = [50, 50, 50]
        image_to_save_classify[np.where((y_argmax_val == 3).all(axis=0))] = [150, 150, 150]
        image_to_save_classify[np.where((y_argmax_val == 4).all(axis=0))] = [25, 100, 0]
        image_to_save_classify[np.where((y_argmax_val == 5).all(axis=0))] = [0, 0, 255]
        image_to_save_classify[np.where((y_argmax_val == 6).all(axis=0))] = [0, 255, 0]
        image_to_save_classify[np.where((y_argmax_val == 7).all(axis=0))] = [255, 0, 0]

    if dataset_used == "french":
        image_to_save_classify[np.where((y_argmax_val == 0).all(axis=0))] = [255, 0, 0]
        image_to_save_classify[np.where((y_argmax_val == 1).all(axis=0))] = [0, 255, 0]
        image_to_save_classify[np.where((y_argmax_val == 2).all(axis=0))] = [34, 139, 34]
        image_to_save_classify[np.where((y_argmax_val == 3).all(axis=0))] = [55, 36, 24]
        image_to_save_classify[np.where((y_argmax_val == 4).all(axis=0))] = [0, 0, 255]

    if not semantic:
        if not test_change:
            path = "{}/{}/result/train/threshold/PyHopper/argmax/argmax_{}_{}.png".format(
                load_models_file_path, loaded_model_name, i, k)
        else:
            path = "{}/{}/result/test/threshold/PyHopper/argmax/argmax_{}_{}.png".format(
                load_models_file_path, loaded_model_name, i, k)
        ...
        if not test_change:
            path = "{}/{}/result/train/threshold/PyHopper/predicted_change/predicted_change_{}_{}.png".format(
                load_models_file_path, loaded_model_name, i, k)
        else:
            path = "{}/{}/result/test/threshold/PyHopper/predicted_change/predicted_change_{}_{}.png".format(
                load_models_file_path, loaded_model_name, i, k)
        ...
        if dataset_used == "vaihingen":
            ...
            temp_array = np.where(temp_array > 128, 0, 255).astype(np.uint8)
        elif dataset_used == "syn":
            temp_array = np.where(y_argmax_val == 7, 255, 0).astype(np.uint8)
            temp_array = temp_array.squeeze(0)
        else:
        ...
        if not test_change:
            path = "{}/{}/result/train/threshold/PyHopper/argmax/argmax_semantic_{}_{}_{}.png".format(
                load_models_file_path, loaded_model_name, semantic_number, i, k)
        else:
            path = "{}/{}/result/test/threshold/PyHopper/argmax/argmax_semantic_{}_{}_{}.png".format(
                load_models_file_path, loaded_model_name, semantic_number, i, k)
...
def displayTruth(y_truth, i, k, loaded_model_name, load_models_file_path, dataset_used, test_change=False):
    ...
    if not test_change:
        imageio.imwrite("{}/{}/result/train/threshold/PyHopper/gtruth/gtruth_change_{}_{}.png".format(
            load_models_file_path, loaded_model_name, i, k), dis)
    else:
        imageio.imwrite("{}/{}/result/test/threshold/PyHopper/gtruth/gtruth_change_{}_{}.png".format(
            load_models_file_path, loaded_model_name, i, k), dis)


def calculateScore(model_output, change_image, dataset_used_score, fitness_function_index, all_metrics=False):
    change_image_pso = change_image.cpu().detach().numpy()
    ...
    if dataset_used_score == "vaihingen":
        ...
        y_argmax_val = np.where(temp_array > 128, 0, 1).astype(np.uint8)
    elif dataset_used_score == "syn":
        y_argmax_val = np.where(y_argmax_val == 7, 1, 0).astype(np.uint8)
    else:
        y_argmax_val[change_image_pso == 0] = 0
        y_argmax_val[y_argmax_val == 2] = 1

    change_image_pso = change_image_pso.transpose(1, 2, 0)

    change_image_pso = change_image_pso.squeeze(2)

    true_positives = np.sum(np.logical_and(y_argmax_val == 1, change_image_pso == 1), dtype='int64')

    true_negatives = np.sum(np.logical_and(y_argmax_val == 0, change_image_pso == 0), dtype='int64')
    false_positives = np.sum(np.logical_and(y_argmax_val == 1, change_image_pso == 0), dtype='int64')
    false_negatives = np.sum(np.logical_and(y_argmax_val == 0, change_image_pso == 1), dtype='int64')
...
def train_Thresholds_Function(generator, did_load_model, parametersObject, param=None):
    ...
    output_every_x_images = parametersObject["output_every_x_images"]  # Output every x images
    ...
    print("PyHopper Training is commencing...")
    ...
    if param is not None:
        threshold_down1_var = param["threshold_one_pyhopper"]
        threshold_down2_var = param["threshold_two_pyhopper"]
        threshold_down3_var = param["threshold_three_pyhopper"]
        threshold_down4_var = param["threshold_four_pyhopper"]
        threshold_bridge_var = param["threshold_five_pyhopper"]
    else:
        threshold_down1_var = np.random.random_sample()
        threshold_down2_var = np.random.random_sample()
        threshold_down3_var = np.random.random_sample()
        threshold_down4_var = np.random.random_sample()
        threshold_bridge_var = np.random.random_sample()

    model.train(False)
    total_difference_score_pso = 0
    total_difference_metrics = []
    total_number_tested = 0
    for l_index_1_pso, (image_pso, second_image_pso, change_image_pso) in enumerate(train_data_loader):
        image_pso = image_pso.to(device, non_blocking=True)
        second_image_pso = second_image_pso.to(device, non_blocking=True)

        with torch.no_grad():
            if not is_running_baseline:
                y_pso = model.forward(inputImage=image_pso, secondImage=second_image_pso,
                                      threshold_down1=threshold_down1_var,
                                      threshold_down2=threshold_down2_var,
                                      threshold_down3=threshold_down3_var,
                                      threshold_down4=threshold_down4_var,
                                      threshold_bridge=threshold_bridge_var)
            ...
            score_pso = calculateScore(copy.deepcopy(y_pso), copy.deepcopy(change_image_pso),
                                       dataset_used, fitness_function_index, True)

        total_difference_score_pso = total_difference_score_pso + score_pso[fitness_function_index]

        if not total_difference_metrics:
            total_difference_metrics = score_pso
        else:
            total_difference_metrics = [sum(x) for x in zip(total_difference_metrics, score_pso)]

        total_number_tested += 1

    difference_value = (total_difference_score_pso / total_number_tested)

    model.train(False)
    total_difference_score_val = 0
    total_difference_metrics_val = []
    number_validated = 0
    for l_index_1, (image_val, second_image_val, change_image_val) in enumerate(validation_data_loader):
        image_val = image_val.to(device, non_blocking=True)
        second_image_val = second_image_val.to(device, non_blocking=True)

        with torch.no_grad():
            if not is_running_baseline:
                y = model.forward(inputImage=image_val, secondImage=second_image_val,
                                  threshold_down1=threshold_down1_var,
                                  threshold_down2=threshold_down2_var,
                                  threshold_down3=threshold_down3_var,
                                  threshold_down4=threshold_down4_var,
            ...
            score_val = calculateScore(copy.deepcopy(y), copy.deepcopy(change_image_val),
                                       dataset_used, fitness_function_index, True)

        if output_images == "True":
            if number_of_classes == 3:
                v_utils.save_image(copy.deepcopy(y),
                                   "{}/{}/result/train/threshold/PyHopper/argmax/gen_image_{}_{}.png".format(
                                       path_to_models,
                                       loaded_model_name,
                                       Utils.global_pyhopper_iteration_number, l_index_1))

            displayGenerated(copy.deepcopy(y), Utils.global_pyhopper_iteration_number, l_index_1, loaded_model_name,
                             path_to_models, dataset_used, change_image_pso=copy.deepcopy(change_image_val))

            displayGenerated(y_before, Utils.global_pyhopper_iteration_number, l_index_1, loaded_model_name,
                             path_to_models, dataset_used, semantic=True, semantic_number=0)
            displayGenerated(y_after, Utils.global_pyhopper_iteration_number, l_index_1, loaded_model_name,
                             path_to_models, dataset_used, semantic=True, semantic_number=1)

            displayTruth(copy.deepcopy(change_image_val), Utils.global_pyhopper_iteration_number, l_index_1,
                         loaded_model_name,
                         path_to_models, dataset_used)
            v_utils.save_image(image_val.cpu().data,
                               "{}/{}/result/train/threshold/PyHopper/before/before_image_{}_{}.png".format(
                                   path_to_models,
                                   loaded_model_name,
                                   Utils.global_pyhopper_iteration_number, l_index_1))
            v_utils.save_image(second_image_val.cpu().data,
                               "{}/{}/result/train/threshold/PyHopper/after/after_image_{}_{}.png".format(
                                   path_to_models,
                                   loaded_model_name,
                                   Utils.global_pyhopper_iteration_number, l_index_1))

        total_difference_score_val = total_difference_score_val + score_val[fitness_function_index]

        if not total_difference_metrics_val:
            total_difference_metrics_val = score_val
        else:
            total_difference_metrics_val = [sum(x) for x in zip(total_difference_metrics_val, score_val)]

        number_validated += 1

    difference_value_val = (total_difference_score_val / number_validated)
    total_difference_metrics_val = [element / float(number_validated) for element in
                                    total_difference_metrics_val]
    print("Validation Score: " + str(difference_value_val) + ", for Iteration: " +
          str(Utils.global_pyhopper_iteration_number) + ", higher is better.")
        ...
        if output_images == "True":
            if number_of_classes == 3:
                v_utils.save_image(copy.deepcopy(y_test),
                                   "{}/{}/result/test/threshold/PyHopper/argmax/gen_image_{}_{}.png".format(
                                       path_to_models,
                                       loaded_model_name,
                                       Utils.global_pyhopper_iteration_number, l_index_1))

            displayGenerated(copy.deepcopy(y_test), Utils.global_pyhopper_iteration_number, l_index_1,
                             loaded_model_name,
                             path_to_models, dataset_used, change_image_pso=copy.deepcopy(change_image_test),
                             test_change=True)

            displayGenerated(y_before_test, Utils.global_pyhopper_iteration_number, l_index_1, loaded_model_name,
                             path_to_models, dataset_used, semantic=True, semantic_number=0, test_change=True)
            displayGenerated(y_after_test, Utils.global_pyhopper_iteration_number, l_index_1, loaded_model_name,
                             path_to_models, dataset_used, semantic=True, semantic_number=1, test_change=True)

            displayTruth(copy.deepcopy(change_image_test), Utils.global_pyhopper_iteration_number, l_index_1,
                         loaded_model_name,
                         path_to_models, dataset_used, test_change=True)
            v_utils.save_image(image_test.cpu().data,
                               "{}/{}/result/test/threshold/PyHopper/before/before_image_{}_{}.png".format(
                                   path_to_models,
                                   loaded_model_name,
                                   Utils.global_pyhopper_iteration_number, l_index_1))
            v_utils.save_image(second_image_test.cpu().data,
                               "{}/{}/result/test/threshold/PyHopper/after/after_image_{}_{}.png".format(
                                   path_to_models,
                                   loaded_model_name,
                                   Utils.global_pyhopper_iteration_number, l_index_1))
    ...
    threshold_values_to_write = "Iteration {}\nthreshold_value_1 = {}\nthreshold_value_2 = {}\nthreshold_value_3 = {}\nthreshold_value_4 = {}\nthreshold_value_5 = {}\n\n".format(
        Utils.global_pyhopper_iteration_number, threshold_down1_var, threshold_down2_var, threshold_down3_var,
        threshold_down4_var, threshold_bridge_var)
    with open('{}/{}/threshold_values_train.txt'.format(path_to_models, loaded_model_name),
              "a") as threshold_values_file:
        threshold_values_file.write(threshold_values_to_write)

    with open('{}/{}/PyHopper Train Info about Models.txt'.format(path_to_models, loaded_model_name),
              'a') as f:
        f.write("Iteration {} Validation Metrics: "
                "\n\tSeed: {}"
                "\n\tTraining Score: {}"
                "\n\tValidation Score: {}"
                "\n\tTest Score: {}"
                ...
                "\n\tSeK: {}"
                "\n\tscore: {}"
                "\n\n".format(Utils.global_pyhopper_iteration_number, seed,
                              difference_value, difference_value_val, difference_value_test,
                              total_difference_metrics_val[0], total_difference_metrics_val[1],
                              total_difference_metrics_val[2], total_difference_metrics_val[3],
                              total_difference_metrics_val[4], total_difference_metrics_val[5],
                              total_difference_metrics_val[6], total_difference_metrics_val[7],
                              total_difference_metrics_val[8], total_difference_metrics_val[9]))

        f.write("Iteration {} Test Metrics: "
                "\n\tSeed: {}"
                "\n\tTraining Score: {}"
                "\n\tValidation Score: {}"
                "\n\tTest Score: {}"
